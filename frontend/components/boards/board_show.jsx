...
class BoardShow extends React.Component {
  ...
  renderButton() {
    if (
      ...
      if (!this.state.deleteButton) {
        return (
          <div className="edit-bar">
            ...
            {this.renderTitle()}
            <button className="animated fadeInUp" onClick={this.handleButton}>
              Organize
            </button>
          </div>
        );
      ...
      return (
        <div className="edit-bar animated fadeInUp">{this.renderTitle()}</div>
      );
    }
  }
  ...
  pinCount() {
    return <div className="pins-count">{this.props.pins.length} Pins</div>;
  }

  noPinsMessage(pins) {
    if (pins.length === 0) {
      return <h1 className="no-pins">You have no pins on this board yet.</h1>;
    } else {
      return null;
    }
  }
  ...
  render() {
    ...
    const board = this.props.board ? this.props.board : {};
